@php
    $title_page=__('admin/agencies/index.title');
@endphp
...
<div class="col-12 p-3">
	...
	<x-bread-crumb :breads="[
			...
			['url' => route('admin.users.index') , 'title' => __('admin/agencies/index.title') , 'isactive' => true],
		]">
		</x-bread-crumb>
	<!-- /breadcrumb -->
		...
		<div class="col-12 px-0">
			<div class="col-12 p-0 row">
				<div class="col-12 col-lg-4 py-3 px-3">
					<span class="fas fa-users"></span>	{{__('admin/agencies/index.title')}}
				</div>
				...
				<div class="col-12 col-lg-4 p-2 text-lg-end">
					...
					<span class="btn btn-primary"><span class="fas fa-plus"></span> {{__('admin/agencies/index.create')}} </span>
					</a>
					@endcan
				</div>
			</div>
		...
		<div class="col-12 p-3" style="overflow:auto">
			...
			<table class="table table-bordered  table-hover">
				<thead>
					<tr>
						<th>#</th>
						<th>{{__('admin/agencies/index.name')}}</th>
						<th>{{__('admin/agencies/index.package')}}</th>
						<th>{{__('admin/agencies/index.status')}}</th>
				...
				<tbody>
					...
					<tr>
						<td>{{$agency->id}}</td>
						<td>
						<?php if(app()->getLocale() == 'ar'){echo $agency->name_ar;}else{echo $agency->name_en;} ?>
						{{$agency->name}}
						</td>
						<td>{{$agency->package_fk->name}}</td>
						<td>
												<?php 
						if($agency->status == 0){
						...
						<td>
							...
							@can('users-delete')
							<form method="POST" action="{{route('admin.agencies.destroy',$agency)}}" class="d-inline-block">@csrf @method("DELETE")
							...
							<div class="dropdown d-inline-block">
								...
								<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1" data-popper-placement="bottom-start" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate3d(0px, 29px, 0px);">
									<?php 
										if($agency->agency_features_fk->count() > 0){ ?>
										<li><a class="dropdown-item font-1" href="{{route('admin.remove.agency.features',$agency)}}"><span class="fal fa-trash"></span> {{__('admin/agencies/index.remove_features')}}</a></li>

									<?php }else{ ?>
										<li><a class="dropdown-item font-1" href="{{route('admin.add.agency.features',$agency)}}"><span class="fal fa-plus"></span> {{__('admin/agencies/index.add_features')}}</a></li>
