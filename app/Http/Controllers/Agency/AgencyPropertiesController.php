...
class AgencyPropertiesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $properties = Properties::where('agency',auth()->user()->id)->with('agent_fk')->paginate(10);
        return view('agencies.properties.index',compact('properties'));
    }
    ...
    public function create()
    {
        ...
        if($check == 'cant'){
            return Redirect::back()->withErrors(['msg' => __('agencies/msgs.cant_add_property')]);
        }else{
            return view('agencies.properties.create',compact('agent','citys','features','types','p_status'));
    ...
    public function propertisFeatures(Request $request)
    {
        $agency = auth('agency')->user();

        $properties =  PropertiesFeatures::where(function($q)use($request){
            if($request->id!=null)
                $q->where('id',$request->id);
            if($request->q!=null)
                $q->where('name','LIKE','%'.$request->q.'%');
        })->with(['agency_fk','property_fk'])->where(['agency' => $agency->id])->orderBy('id','DESC')->paginate(10);
       
        return view('agencies.properties.properties-features',compact('properties'));
    }

    public function propertisPending(Request $request)
    {
        $agency = auth('agency')->user();
        ...
        $properties =  Properties::where(function($q)use($request){
            ...
            if($request->q!=null)
                $q->where('name','LIKE','%'.$request->q.'%');
        })->with(['agency_fk','agent_fk','property_features_fk'])->where(['agency' => $agency->id,'activity_status' => 1])->orderBy('id','DESC')->paginate(10);

        return view('agencies.properties.properties-pending',compact('properties'));
    }
