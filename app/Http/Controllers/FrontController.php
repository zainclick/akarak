...
class FrontController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $prop_feaures = PropertiesFeatures::with('property_fk')->paginate(6);
        $properties_rents = Properties::where(['property_status' => 1])->with('property_imgs_fk','status_fk')->get();
        $properties_sales = Properties::where(['property_status' => 2])->with('property_imgs_fk','status_fk')->get();
        $agents_feaures = AgentsFeatures::with('agent_fk','agency_fk')->paginate(4);
        $articles = Article::where('is_featured',1)->paginate(3);
        $agencies_feaures = AgenciesFeatures::with('agency_fk')->paginate(6);

        return view('front.index',compact('prop_feaures','properties_rents','properties_sales','citys','agents_feaures','agencies_feaures','articles','types','features'));
    }


    public function test(){
        $agency = auth('agency')->user();
        if ($agency->subscription('default')->onGracePeriod()) {
            return 'true';
        }else{
            return 'false';
        }
